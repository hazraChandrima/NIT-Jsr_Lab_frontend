...
function EditStudentForm() {
  ...
  const [formData, setFormData] = useState({
    name: '', roll: '', degree: 'Ph. D', about: '', profilePhoto: '', resume: '',
  });
  ...
  useEffect(() => {
    ...
    if (!studentId) { router.push('/admin/students'); return; }
    setIsAdminLoggedIn(true);
    fetchStudent();
  }, [isLoggedIn, isLoading, router, studentId]);
  ...
  const fetchStudent = async () => {
    try {
      ...
      if (a) {
        setFormData({
          name: a.name || '', roll: a.roll || '', degree: a.degree || 'Ph. D',
          about: a.about || '',
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (!response.ok) { const d = await response.json(); throw new Error(d.error || 'Failed to update student'); }
  ...
  if (loading || isLoading) {
    return (<div className="min-h-screen flex items-center justify-center bg-gray-50"><div className="text-2xl text-gray-700 animate-pulse">Loading student data...</div></div>);
  }
  ...
  return (
    <div className="min-h-screen bg-gray-50 py-12 px-4">
      <div className="max-w-3xl mx-auto bg-white rounded-xl shadow-lg p-8">
        <h1 className="text-3xl font-bold text-gray-900 mb-4">Edit Student</h1>
        <p className="text-gray-600 mb-8">Update the student details below</p>
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className="grid gap-4 sm:grid-cols-2">
            <FileUpload label="Profile Photo" accept="image/*" folder="mvi_lab/students" value={formData.profilePhoto} onChange={(url) => setFormData((prev) => ({ ...prev, profilePhoto: url }))} />
